...
/* Compara X.Y.Z aunque el helper del boot aún no esté disponible (tests / orden de carga). */
...
function betaChecklist(version,prodVersion){
  ...
  /* Con Pages conocida, revisar solo la última versión dejaba invisibles tandas que estaban en
     esta misma beta. Sin red conservamos una única versión: si falta la referencia segura, no
     inventamos una ronda. */
  var conProd=prodVersion!=null&&prodVersion!=="", round;
  if(!conProd){
    var one=RELEASE_NOTES.filter(function(n){ return n.v===base; })[0]||RELEASE_NOTES[0];
    round=one?[one]:[];
  }else{
    var prod=mcVerBase(prodVersion);
    round=RELEASE_NOTES.filter(function(n){ return n&&n.v&&mcIsNewer(n.v,prod)&&!mcIsNewer(n.v,base); });
  }
  if(!round.length){
    var fallback=RELEASE_NOTES.filter(function(n){ return n.v===base; })[0]||RELEASE_NOTES[0];
    round=fallback?[fallback]:[];
  }
  ...
  round.forEach(function(notes){
    ...
    betaTandas(notes).forEach(function(g){
      /* Dos tandas de versiones distintas pueden compartir id: prefijarlo evita que sus
         veredictos se pisen. Para una sola versión se conserva la clave ya guardada. */
      tandas.push({id:conProd?(notes.v+"/"+g.id):g.id,t:conProd?("v"+notes.v+(g.t?" · "+g.t:"")):g.t,items:g.items});
...
/* Pages responde con su versión cruda; compartirla evita dos consultas que podían discrepar. */
...
function BetaReviewPanel({onClose, showToast}){
  ...
  const yaEnProd=(!prod||!/^\d+\.\d+\.\d+$/.test(mcVerBase(CONFIG.APP_VERSION)))?null:(!mcIsNewer(mcVerBase(CONFIG.APP_VERSION),prod)?prod:false);
...
var RELEASE_NOTES=[
  {v:"4.18.14",d:"11 sep 2026",
   t:{es:"Revisar la beta muestra la ronda entera",en:"Beta review shows the whole round",ca:"Revisar la beta mostra tota la ronda"},
   tandas:[{id:"panel-ronda",t:{es:"🧪 Toda la ronda a la vista",en:"🧪 Whole round in view",ca:"🧪 Tota la ronda a la vista"},items:{
     es:["Ajustes → Revisar esta beta reúne las tandas pendientes de toda la ronda, con su versión en el título."],
     en:["Settings → Review this beta gathers the pending tandas from the whole round, with their version in the title."],
     ca:["Ajustaments → Revisar aquesta beta reuneix les tandes pendents de tota la ronda, amb la versió al títol."]}}],
   items:{es:["Revisar la beta reúne toda la ronda pendiente, con pasos claros."],en:["Beta review gathers the full pending round, with clear steps."],ca:["Revisar la beta reuneix tota la ronda pendent, amb passos clars."]}},
...
function SettingsPanel({state, set, onClose, showToast, uid, onBankSync, onTour, totals, fetchPrices, refreshFx, goBanks, goBanksFocus, goGastos}){
  ...
  const prodVer=useProdVersion();                // Pages ahora; null mientras pregunta
  const yaEnProd=(!prodVer||!/^\d+\.\d+\.\d+$/.test(mcVerBase(CONFIG.APP_VERSION)))?null:(!mcIsNewer(mcVerBase(CONFIG.APP_VERSION),prodVer)?prodVer:false);
