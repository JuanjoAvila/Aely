...
function BetaReviewPanel({onClose, showToast}){
  useBackClose(true, onClose);
  /* La checklist SON las notas de esta versión, y ahora llegan por fetch (NOTAS-BUNDLE). Sin
     esperarlas, el panel se abriría vacío y parecería que la ronda no tiene nada que probar:
     exactamente el fallo mudo contra el que avisa el comentario de arriba. */
  ...
  const pack=notesReady?betaChecklist(CONFIG.APP_VERSION):{ v:mcVerBase(CONFIG.APP_VERSION), t:"", items:[], tandas:[] };
...
/* NOTAS-BUNDLE — EL HISTÓRICO YA NO VIAJA DENTRO DEL JS (portado de beta a producción el 11/9).
   Las 92 versiones vivían pegadas aquí y pesaban ~115 KB en el módulo. Con la ronda de prod del
   11/9 el gzip del index llegó a 343,8 KB de un tope de 344: a 0,2 KB de reventar, y el gzip es
   lo que de verdad baja al móvil. Portar una tanda más habría sido imposible sin esto.

   Ahora la fuente es `src/data/release-notes.json`; el build lo copia a `public/release-notes.json`
   y deja este array VACÍO a propósito. `ensureReleaseNotes()` lo rellena al abrir Novedades o el
   panel de revisión. El bundle del móvil lo lleva igual: `build-www.mjs` copia `public/` entero.

   ⚠ NO vuelvas a pegar el histórico aquí, y NO edites las notas a mano: se tocan con el parser
   del repo (`scripts/release-notes-max.mjs`). Hacerlo a mano ya desactivó el tope en silencio
   una vez y le coló +72 KB a toda la familia sin que ningún test dijera nada. */
/* ⚠ LO QUE SE PERDIÓ AL SACAR LAS NOTAS A JSON, RESCATADO AQUÍ (11/9).
   El histórico llevaba dentro cinco comentarios con la práctica de la casa. Un JSON no admite
   comentarios, así que la migración se los habría comido en silencio; lo cazó `season-detalle`,
   que vigilaba uno de ellos. Van aquí porque es donde se edita y donde hay que leerlos:

   · UNA TANDA APROBADA SE BORRA DEL ARRAY, NO SE MARCA HECHA. Si se deja «marcada», el panel de
     revisión se la sigue pidiendo cada vez que abre Ajustes. Historial de bajas: `import` → 4.12.4;
     `arranque` → 4.12.3; `tutorial-gestos` quitada el 4/8; `bancos`, `temporada`, `reservar` e
     `import-docx-pdf` quitadas el 4/8; `gestos` y `plan-swipe` el 5/8; multidivisa y
     `gastos-fx-overflow` el 5/8 al subirlas a producción; la ronda del 6/8 el mismo día.

   · LOS PUNTOS DE UNA TANDA NO SE REESCRIBEN SI SE REPITE EN OTRA COMPILACIÓN. El panel hereda
     los ✓ y los ✗ casando por el TEXTO del punto: cambiar una coma le borra lo que ya había
     probado y se lo hace repetir.

   · LO QUE YA SE PROMOCIONÓ SOLO, NO SE VUELVE A CONTAR. Cuando una parte de una ronda sube a
     producción por su cuenta (4.12.3 arranque, 4.12.4 importar hoja), sus puntos NO vuelven a la
     nota del resto de la ronda: sería contarle a la familia la misma novedad dos veces. */
...
/* Idempotente. Si la descarga falla se devuelve lo que haya (vacío) y se deja reintentar: nunca
   se lanza hacia arriba, que aquí una excepción se comería el popup entero. */
...
function WhatsNew({onClose, showToast, set, state}){
  useBackClose(true, onClose);
  /* El histórico llega por fetch (NOTAS-BUNDLE), así que de entrada puede estar vacío. */
  ...
  useEffect(function(){
    ...
    ensureReleaseNotes().then(function(arr){
      if(!alive) return;
      setNotes(arr.slice());
      setOpenV(function(v){ return v || (arr.length?arr[0].v:null); });
    });
    return function(){ alive=false; };
  },[]);
