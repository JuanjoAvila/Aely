...
| **UX-06 · pulido visual v4 (PULIDO-v4)** · **BLOQUEADO: falta el fichero** | Encargo del dueño el 9/9 desde Claude Design: **14 tareas P1–P14 + 5 bonus B1–B5** de acabado sobre la v4 que ya está en producción. No es rediseño. El brief trae, tarea por tarea, el fichero y el selector/función exactos, y una cabecera de ficheros que NO se pueden tocar. **`docs/design/handoff/PULIDO-v4.md` NO existe en el repo** (comprobado: en `docs/design/handoff/` solo hay README, SPEC-v4, capturas y el mockup). Sin él no se puede empezar. | Reglas que ya dio: recorrer el brief y confirmar **una línea por tarea** de que cada punto se localiza en el código y si sigue aplicando, antes de tocar nada — si algo no está donde dice, avisar, no buscar «lo parecido». Orden P1–P5 (primer minuto de un usuario nuevo), luego P6–P14; los bonus **solo con su OK**. **UN COMMIT POR TAREA** con el id delante («P1: …»). Cero refactors y cero reformateo; única excepción autorizada, P9: extraer un `<NumPad>` compartido a `02-ui-shared.js` desde `14-v4-screens.js` y `09-tab-debts-goals.js`, mismo markup y clases. Nada de datos de ejemplo ni PII. Al acabar, su checklist de 4 puntos (reduced-motion; 360x667 y 430x932 con y sin «Letra grande»; app recién instalada; guardianes de rendimiento) marcada una por una, **diciendo lo que NO se ha podido probar**. Si una tarea choca con trabajo en curso: no forzarla, anotarla y seguir. |
