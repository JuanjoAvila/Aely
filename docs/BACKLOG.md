...
| **OPS-03 · P2 · beta con varios probadores** | Un canal beta y panel existen; falta demostrar acceso/veredictos independientes de varios usuarios sin hacerlos administradores. | Diseñar alta de probador y matriz de permisos; dos cuentas de pruebas, veredictos separados y cambio de canal fiable. Sin tercer canal Experimental. Flags solo si una necesidad concreta lo exige. |
...
| **OPS-05 · P2 · higiene de repositorio y PR** | Hay PR abiertas con cambios ya incorporados y documentación valiosa fuera de beta; ver matriz abajo. | Comparar commits/diffs, rescatar solo contenido vigente y documentar equivalencia antes de cerrar PR. Retirar sondas/código muerto con evidencia. Ramas/worktrees requieren permiso para borrar. No refactorizar dinero dentro de esta limpieza. |
...
| **UX-04 · sugerencias sin cierre fiable** | Contrastado con `npm run sugerencias` el 9/9: quedan 7 eventos. **El mensaje al actualizar cuentas está atendido**: `bank_upd_one/n` = «✓ {banco} al día · {x}» / «✓ {n} bancos al día» en los tres idiomas (`01-i18n.js:1880/1899/1918`, usado en `11-app-main.js:501`). Es claro y conciso, que era la petición del 26/7. El acceso tocando la cartera superior (de su pareja, 16/7, v3.109) sigue ambiguo: hoy el avatar abre el perfil y Ajustes se abre deslizando desde el borde. **Preguntado al dueño el 9/9.** | Registrar el cierre del mensaje salvo feedback nuevo. Para el acceso superior, localizar pantalla/intención antes de proponer cambio; si sigue ambiguo, aclaración puntual al dueño. La mera presencia de feedback viejo en la tabla no significa que siga sin atender. |
...
| Rechazo anterior; revalidar con evidencia | `4.19.2/ventana-mes`, `4.19.1/avisos-presupuesto`, `4.19.0/tr-reactivo` |
...
|---|---|
| [#41](https://github.com/JuanjoAvila/Mi-Cartera/pull/41), higiene checkout → beta | Regla de worktrees y `git switch beta` recogida en la entrada. Comparar diff antes de integrar/cerrar para no duplicar documentación. |
| [#35](https://github.com/JuanjoAvila/Mi-Cartera/pull/35), efectivo → beta; [#36](https://github.com/JuanjoAvila/Mi-Cartera/pull/36), edit-link → rama de Claude | Efectivo ya está integrado. Comprobar equivalencia exacta y posibles restos de doc/tests; no reintroducir código anterior sobre FIN-01 o 4.19.14. |
| [#31](https://github.com/JuanjoAvila/Mi-Cartera/pull/31), resume/categorías → beta | Código integrado. Los límites del protocolo siguen FIN-03/04/05; el backfill inseguro fue retirado, no es un fix pendiente de reactivar. |
| [#30](https://github.com/JuanjoAvila/Mi-Cartera/pull/30), arquitectura → beta | Arquitectura recuperada en este relevo; resto de diagnósticos son evidencia histórica. No arrastrar estado de versiones/roles del 8/9 sobre el actual. |
| [#26](https://github.com/JuanjoAvila/Mi-Cartera/pull/26), canal/incidencias → beta | Rescatar evidencia no duplicada y reconciliar protocolo con el relevo actual (Claude dirige). La contención está integrada; recuperación/identidad no terminadas. |
| [#24](https://github.com/JuanjoAvila/Mi-Cartera/pull/24), huecos conocidos → main | Su backlog ya está cruzado aquí. Estado de versiones antiguo; no fusionar a ciegas. Su frase sobre seguridad «directo main» NO autoriza desplegar backend compartido. |
