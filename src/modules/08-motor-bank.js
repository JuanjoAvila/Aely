...
function applyBankBalances(s, links){
  ...
  /* Gasto del mes PARTIDO POR BANCO, calculado UNA vez y de la misma lista que usa el pintado.
     Si se recalculara dentro del bucle con el total del mes, un segundo banco diario volvería a
     comerse los gastos ajenos — que es exactamente el bug del saldo de su padre. */
  ...
  const porBanco=gastoDelMesPorBanco(monthExp, dailyAcc&&dailyAcc.ent);
  links.forEach(function(lk){
    ...
    accs.forEach(function(ac,k){
      ...
      if(ownerOf[k]!=null){
        ...
        /* re-anclaje = despejar `value` para que HOY se pinte el saldo real del banco.
           MISMA fórmula que al pintar (`saldoCuentaMostrada` ↔ `valueDesdeSaldo`). Antes `spentM`
           sumaba el gasto del mes de TODOS los bancos y, con rol diario/ambos, inflaba la base:
           a su padre el banco le decía 26,46 € y la app le enseñaba 455,50 € porque se le
           devolvían a Revolut los gastos que hace con CaixaBank y Trade Republic (2026-09-11).
           Ya estaba arreglado en la mitad que PINTA desde agosto (`gastoDelMesPorBanco`, cuando
           un cargo de Revolut se comió 257,17 € de TR); esta era la copia que se quedó sin migrar. */
...
function importObExpenses(s, txs){
  ...
  /* Lápidas: «es el mismo» borra la fila OB y deja clave para que el siguiente sync no la reviva. */
  const delSet={}; (s.deleted||[]).forEach(function(k){ delSet[k]=1; });
  /* Un Movimiento sin nombre puede ser una notificación ya vista o una compra distinta. Nunca se
     descarta: si coincide con una vía no-OB del MISMO banco, entra marcada para decidirlo. */
