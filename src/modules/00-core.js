...
/* FIN-07 — bucle de páginas puro (testeable sin Supabase). `fetchPage(cursor)` recibe
   null la primera vez y luego `{fecha,id}` del último de la página anterior (orden
   fecha desc, id desc). Corta cuando una página viene corta; si agota maxPages → capped. */
...
const cloud = (function(){
  ...
  return {
    ...
    /* FIN-07 — histórico cloud completo (10/9). Antes: un solo .limit(2000). Con tope,
       syncCloudExpenses REEMPLAZA los gastos source=supabase por lo bajado: los más
       viejos desaparecían del móvil aunque siguieran en la nube («solo baja un poquito»
       / falsos nuevos en el histórico sin ext_id en memoria). Paginación por keyset
       (fecha desc, id desc) — no OFFSET: altas concurrentes no deben abrir huecos.
       Tope de seguridad MAX_PAGES×PAGE: si se llena, _mcPullCapped=true y el toast sigue. */
    async pullExpenses(){
      if(!sb) return [];
      const PAGE=1000;
      const MAX_PAGES=50;   // 50k filas: red de seguridad, no techo de producto
      const fetchPage=async function(cursor){
        let q=sb.from("expenses").select("*")
          ...
          .limit(PAGE);
        if(cursor){
          // Estrictamente «después» en orden desc: fecha menor, o misma fecha e id menor.
          const f=String(cursor.fecha);
