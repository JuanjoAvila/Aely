...
| **Widget · elección de tarjeta/cuenta · petición 26/9** | Al usuario le aparece CaixaBank en «Puedes gastar en» y quiere poder elegir otra tarjeta o cuenta. Tarea separada de FIN-05, apuntada para la próxima mañana; hoy no se modifica el comportamiento. | Comprobar primero si ya existe un selector y dónde se guarda; permitir la elección explícita, persistirla y verificar qué muestra el widget en móvil. Mantener separados el banco mostrado y los cálculos de presupuesto/saldo para no inventar ni duplicar importes. Beta antes de producción y `main` solo con aprobación. |
