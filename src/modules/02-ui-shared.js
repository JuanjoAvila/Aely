...
/* Iconos del launcher (su captura). Fondo y trazo como en el móvil. */
function bankMark(id){
  if(id==="sabadell"){
    /* Blanco: bola azul con B + S negra (icono Banco Sabadell del launcher). */
    return {bg:"#FFFFFF",scale:0.88,el:React.createElement(React.Fragment,null,
      React.createElement("circle",{cx:6.4,cy:12,r:4.5,fill:"#0082C9"}),
      React.createElement("path",{fill:"#fff",d:"M4.35 8.9h2.15c1.1 0 1.78.58 1.78 1.42 0 .52-.3.95-.8 1.15.78.24 1.2.74 1.2 1.5 0 1-.82 1.63-2.1 1.63H4.35V8.9zm1.4 2.25h.82c.5 0 .78-.24.78-.62s-.28-.58-.78-.58h-.82v1.2zm0 2.85h1c.58 0 .92-.28.92-.72s-.34-.7-.92-.7h-1v1.42z"}),
      React.createElement("text",{x:16.2,y:17.2,textAnchor:"middle",fill:"#111",
        fontSize:16,fontWeight:800,fontFamily:"system-ui,-apple-system,sans-serif"},"S")
    )};
  }
  if(id==="revolut"){
    /* Negro + R blanca (icono Revolut del launcher). Trazo simple-icons. */
    return {bg:"#0B0B0B",fg:"#fff",scale:0.58,el:React.createElement("path",{d:"M20.913 6.957C20.913 3.121 17.79 0 13.95 0H2.424v3.86h10.978c1.738 0 3.177 1.365 3.209 3.043.016.84-.3 1.633-.888 2.233-.589.6-1.375.93-2.214.93H9.232a.276.276 0 0 0-.276.275v3.431c0 .059.018.114.052.161L16.265 24h5.311l-7.273-10.094c3.663-.184 6.61-3.261 6.61-6.949zM6.894 5.923H2.424V24h4.47z"})};
  }
  if(id==="trade_republic"){
    /* Negro: 3 trazos blancos — arriba/abajo ondulados, el del medio recto (icono TR). */
    return {bg:"#0B0B0B",scale:0.72,el:React.createElement(React.Fragment,null,
      React.createElement("path",{d:"M3.2 7.2 L8.2 4.6 L12 7.2 L15.8 4.6 L20.8 7.2",fill:"none",stroke:"#fff",strokeWidth:2.05,strokeLinecap:"round",strokeLinejoin:"round"}),
      React.createElement("path",{d:"M3.2 12 H20.8",fill:"none",stroke:"#fff",strokeWidth:2.05,strokeLinecap:"round"}),
      React.createElement("path",{d:"M3.2 16.8 L8.2 14.2 L12 16.8 L15.8 14.2 L20.8 16.8",fill:"none",stroke:"#fff",strokeWidth:2.05,strokeLinecap:"round",strokeLinejoin:"round"})
    )};
  }
  if(id==="myinvestor"){
    /* Blanco: «my» negro + barra vertical degradado (icono MyInvestor — SIN círculo). */
    return {bg:"#FFFFFF",scale:0.9,el:React.createElement(React.Fragment,null,
      React.createElement("text",{x:9.2,y:15.6,textAnchor:"middle",fill:"#1a1a1a",
        fontSize:11,fontWeight:700,fontFamily:"system-ui,-apple-system,sans-serif",letterSpacing:"-0.6px"},"my"),
      React.createElement("defs",null,
        React.createElement("linearGradient",{id:"miBar",x1:"0",y1:"0",x2:"0",y2:"1"},
          React.createElement("stop",{offset:"0%",stopColor:"#7B2D8E"}),
          React.createElement("stop",{offset:"55%",stopColor:"#E6007E"}),
          React.createElement("stop",{offset:"100%",stopColor:"#2F6BFF"})
        )
      ),
      React.createElement("rect",{x:15.6,y:5.2,width:1.55,height:13.6,rx:0.75,fill:"url(#miBar)"})
    )};
  }
  if(id==="caixabank"){
    /* Blanco: estrella Miró + puntos (icono CaixaBankNow). */
    return {bg:"#FFFFFF",scale:0.88,el:React.createElement(React.Fragment,null,
      React.createElement("path",{fill:"#00A0DF",d:"M13.1 2.4l1.35 5.5 5.05-.2-3.85 3.35 2.55 4.55-4.85-1.95-3.55 3.75.15-5.25-4.65-2.35 5.35-.7z"}),
      React.createElement("circle",{cx:16.6,cy:15.2,r:1.35,fill:"#FFCC00"}),
      React.createElement("circle",{cx:16.9,cy:19.1,r:1.95,fill:"#E30613"})
    )};
  }
  return null;
}
