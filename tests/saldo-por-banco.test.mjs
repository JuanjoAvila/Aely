...
t("7. re-anclar Revolut con DOS bancos: el pintado cuadra con el banco (padre 26,46)", () => {
  ...
  /* En producción todavía no existe `saldoCuentaMostrada` (llegó en beta), así que se pinta con
     `shownDaily`, que es el mismo `saldoCuentaGasto` que usa la app para una cuenta diaria/ambos.
     Lo que se prueba es idéntico: que la base re-anclada, al pintarla, dé el saldo del banco. */
  const shown = shownDaily(acc, {
    injTR: 0, spentByBank: spentByBank, paidNet: paidNet,
